...
from ring_gc_h_iden_19Ghz.cell_euler import RingModulatorGCHiepIden_19GHz_Euler, RingModulatorGCHiepIden_19GHz_2_Euler
from ring_gc_h_iden_19Ghz_heater_not_on_top.cell_euler import RingModulatorGCHiepIden_19GHz_heater_not_on_top_Euler

from ring_modulator_h_iden_19Ghz.cell_euler import AddDropRingWithElectrodeHiepIden_19GHz_Euler, AddDropRingWithElectrodeHiepIden_19GHz_2_Euler
from ring_modulator_h_iden_19Ghz.cell_euler import AddDropRingWithElectrodeHiepIden_19GHz_heater_not_on_top_Euler
from ring_modulator_h_iden_19Ghz_heater_changed.cell_euler import AddDropRingWithElectrodeHiepIden_19GHz_heater_changed_v2_option_3_Euler
from ring_gc_h_iden_19Ghz_heater_changed.cell_euler import RingModulatorGCHiepIden_19GHz_heater_changed_v2_option_3_Euler


from ring_modulator.cell_euler import AddDropRingWithElectrode, AddDropRingWithOutElectrode
from ring_gc.cell_euler import RingModulatorGC, RingModulatorGC_NoElectrode
...
######################################
coupling_gaps = 1.25
topo_gaps = [0.9, 0.95, 1.0] # Topology coupler gap
...
ring_19_GHz = AddDropRingWithElectrodeHiepIden_19GHz_heater_not_on_top_Euler()
...
first_ring_19_GHz_position = (-ring_19_GHz_in_pos[0] + bend_radius - 4500+360-341+63-410+611+25, -1500+50+150-25)
...
ring_gc_h = RingModulatorGCHiepIden_19GHz_heater_not_on_top_Euler(name=name,
...
ring_gc_lo_h = ring_gc_h.Layout(ring_radius=ring_radius,
                                ...
                                coupler_straight_length=coupl_length,
                                topo_coupler_straight_length=topo_coupl_length,
                                coupler_positions=[-straight_length * 0.125 - 750 + coupl_length+250-100-100,
                                                   straight_length * 0.2 - 700 - coupl_length+250+90],
...
first_ring_19_GHz_position = (-ring_19_GHz_in_pos[0] + bend_radius - 4500+360-341+63-410+611+25, -1500 + 50-187+250-25)
...
ring_19_GHz = AddDropRingWithElectrodeHiepIden_19GHz_2_Euler()
...
first_ring_19_GHz_position = (-ring_19_GHz_in_pos[0] + bend_radius - 4500-360-150-410+25, -1500 + 153-25)
...
ring_position = (first_ring_19_GHz_position[0] + ring_width_2 * i - fibre_array_pich * 4 * i + waveguide_spacing * i+4000, first_ring_19_GHz_position[1])
name = "Ring_19GHz_EG_{}G_{}_TOPO_G{}".format(electrode_gap, gap,topo_gap)
ring_gc_h = RingModulatorGCHiepIden_19GHz_2_Euler(name=name,
                                ring_position=ring_position,
                                gc_wg_distance=-ring_position[1] - ring_radius - 100 - bend_radius * 2 - waveguide_spacing * 4 * i,
                                ring_wg_distance=bend_radius + 5,
                                bend_radius=bend_radius,
...
ring_gc_lo_h = ring_gc_h.Layout(ring_radius=ring_radius,
                                ...
                                coupler_straight_length=coupl_length,
                                topo_coupler_straight_length=topo_coupl_length,
                                coupler_positions=[-straight_length * 0.125 - 750 + coupl_length+350-50,
                                                   straight_length * 0.2 - 700 - coupl_length+350+150+50],
                                electrode_length=straight_length - 750,
                                hot_width=hot_electrode_width,
                                electrode_gap=electrode_gap,
                                taper_gap = electrode_gap,
...
coupling_gaps = 1.25
topo_gaps = [1.05, 1.1, 1.0] # Topology coupler gap
...
ring_19_GHz = AddDropRingWithElectrodeHiepIden_19GHz_Euler()
...
first_ring_19_GHz_position = (-ring_19_GHz_in_pos[0] + bend_radius - 4500-360+10-10-410+611+25, -1500 + 50+150-25)
...
ring_position = (first_ring_19_GHz_position[0] + ring_width_2 * i - fibre_array_pich * 4 * i + waveguide_spacing * i+4000, first_ring_19_GHz_position[1])
name = "Ring_19GHz_EG_{}G_{}_TOPO_G{}".format(electrode_gap, gap,topo_gap)
ring_gc_h = RingModulatorGCHiepIden_19GHz_Euler(name=name,
                                ring_position=ring_position,
                                gc_wg_distance=-ring_position[1] - ring_radius - 100 - bend_radius * 2 - waveguide_spacing * 4 * i,
                                ring_wg_distance=bend_radius + 5,
                                # coupler_bend_radius=coupler_bend_radius,
...
ring_gc_lo_h = ring_gc_h.Layout(ring_radius=ring_radius,
                                ...
                                coupler_straight_length=coupl_length,
                                topo_coupler_straight_length=topo_coupl_length,
                                coupler_positions=[-straight_length * 0.125 - 750 + coupl_length+250-100-100-90,
...
first_ring_19_GHz_position = (-ring_19_GHz_in_pos[0] + bend_radius - 4500-360-150-410+611+25, -1500 + 153-25-15)
...
ring_gc_h = RingModulatorGCHiepIden_19GHz_heater_changed_v2_option_3_Euler(name=name,
                                ring_position=ring_position,
                                gc_wg_distance=-ring_position[1] - ring_radius - 100 - bend_radius * 2 - waveguide_spacing * 4 * i,
                                ring_wg_distance=bend_radius + 5,
                                bend_radius=bend_radius,
...
ring_gc_lo_h = ring_gc_h.Layout(ring_radius=ring_radius-10,
                                ...
                                coupler_straight_length=coupl_length,
                                topo_coupler_straight_length=topo_coupl_length,
                                coupler_positions=[-straight_length * 0.125 - 750 + coupl_length+350-230-120,
                                                   straight_length * 0.2 - 700 - coupl_length+350-50],
                                electrode_length=straight_length - 750,
                                # hot_width=hot_electrode_width,
                                electrode_gap=electrode_gap,
                                # taper_gap = electrode_gap,
...
first_ring_19_GHz_position = (-ring_19_GHz_in_pos[0] + bend_radius - 4500-800-800-50-410+150+25, -1500 + 50-895+25-50-450+1600+50)
...
ring_gc_lo_h = ring_gc_h.Layout(ring_radius=ring_radius_test,
                                ...
                                coupler_spacing=gap,  # Coupler gap
                                coupler_straight_length=coupl_length,
                                coupler_positions=[-straight_length_test * 0.125 - 150 + coupl_length+250-25-50,
                                                   straight_length_test * 0.2 - coupl_length+250-25+140],
...
gc_position = (3130 + moving - 1325-150 + 420 + 55-1000-127*2-450+4900-350+127*6,  9250+400-3000-295-65-300+50+450-3000+250-100)  # Position of the first grating coupler
...
ring_gc_lo_h = ring_gc_h.Layout(ring_radius=ring_radius_test,
                                ...
                                coupler_positions=[-straight_length_test * 0.125 - 150 + coupl_length+250,
                                                   straight_length_test * 0.2 - coupl_length+250+200],
                                electrode_length=straight_length_test - 150,
                                hot_width=hot_electrode_width,
                                electrode_gap=electrode_gap,
                                # with_label=True,
...
ring_gc_lo_h = ring_gc_h.Layout(ring_radius=ring_radius_test,
                                ...
                                coupler_straight_length=coupl_length,
                                coupler_positions=[-straight_length_test * 0.125 - 150 + coupl_length+250,
                                                   straight_length_test * 0.2 - coupl_length+250+170],
                                electrode_length=straight_length_test - 150,
                                hot_width=hot_electrode_width,
                                electrode_gap=electrode_gap,
                                # with_label=True,
...
chip_layout.write_gdsii("gds_output/two_euler_chips_iden_20GHz_with_heaters_1.40_v1_new_heater_design.gds")