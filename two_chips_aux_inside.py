...
from ring_gc_h_with_auxilary_ring_inside.cell import RingModulatorGCHiepAuxInside
from ring_gc_h_with_auxilary_ring_inside_2.cell import RingModulatorGCHiepAuxInside2
from ring_modulator.cell import AddDropRingWithElectrode
from ring_modulator_h2.cell import AddDropRingWithElectrodeHiep2
from ring_modulator_with_auxilary_ring.cell import AddDropRingWithAuxInside
#from heater import Heater_Hiep
...
print("Ring straight length auxilary: {}".format(straight_length_aux))
...
#Coupling gap between main ring and auxiliary ring
aux_coupling_gap = 0.8

change = 150
moving = 250 + change
gc_position = (3130 + moving, 9250+225-500)  # Position of the first grating coupler
...
first_ring_position = (-ring_in_pos[0] + bend_radius-700, -1850 + 375)


ring_9_5_GHz = AddDropRingWithAuxInside()
...
first_ring_9_5_GHz_position = (-ring_9_5_GHz_in_pos[0] + bend_radius +3690, -1850 + 375)

ring_aux = AddDropRingWithAuxInside()

ring_aux_lo = ring_aux.Layout(ring_straight_length=straight_length_2,
                      coupler_spacing=coupling_gaps[0], # Coupler gap
                      coupler_straight_length=0,
                      electrode_length=straight_length_2 - 150,
                      hot_width=hot_electrode_width,
...
ring_aux_in_pos = ring_aux_lo.ports['in'].position

first_aux_ring_position = (-ring_in_pos[0] + bend_radius, -1850 + 50)
...
ring_gc_h = RingModulatorGCHiepAuxInside2(name=name,
...
ring_gc_lo_h = ring_gc_h.Layout(aux_coupling_gap = aux_coupling_gap,
                                ...
                                coupler_positions=[-straight_length * 0.125 - 100 + coupl_length,
                                                   straight_length * 0.2 - 50 - coupl_length],
                                electrode_length=straight_length - 350,
                                hot_width=hot_electrode_width,
                                electrode_gap=electrode_gap,
                                with_label=True,
...
i = 1
gap = coupling_gaps[1]
ring_position = (first_ring_9_5_GHz_position[0] + ring_width_aux * i - fibre_array_pich * 4 * i + waveguide_spacing * i, first_ring_9_5_GHz_position[1])
name = "Ring_9_EG_{}G_{}".format(electrode_gap, gap)
ring_gc_h = RingModulatorGCHiepAuxInside(name=name,
                                ring_position=ring_position,
                                gc_wg_distance=-ring_position[1] - ring_radius - 50 - bend_radius * 2 - waveguide_spacing * 4 * i,
                                ring_wg_distance=bend_radius + 50,
                                bend_radius=bend_radius,
...
ring_gc_lo_h = ring_gc_h.Layout(aux_coupling_gap = aux_coupling_gap,
                                ...
                                coupler_spacing=gap,  # Coupler gap
                                coupler_straight_length=coupl_length,
                                coupler_positions=[-straight_length_2 * 0.125 + coupl_length,
                                                   straight_length_2 * 0.2 + 100 - coupl_length],
                                electrode_length=straight_length_2 - 350,
                                hot_width=hot_electrode_width,
                                electrode_gap=electrode_gap,
                                with_label=True,
                                label_position=ring_position,
...
change = 150
moving = 250 + change
gc_position = (3130 + moving, 7050+150-220)  # Position of the first grating coupler
...
chip_layout.write_gdsii("gds_output/two_chips_aux_inside.gds")