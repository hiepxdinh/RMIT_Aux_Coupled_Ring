...
class AddDropRingWithNoElectrode(i3.PCell):
    """
    ...
    ring_aux = i3.ChildCellProperty(locked=True, doc="the ring")
    ...
    class Layout(i3.LayoutView):
        ...
        def _default_ring_aux(self):
            ...
            ring_aux_cell_layout = self.cell.ring_aux.get_default_view(i3.LayoutView)
        ...
        def _generate_instances(self, insts):
            ...
            insts += i3.SRef(name='ring2', reference=self.ring_aux, position=(self.ring_straight_length*0.5 -self.ring_straight_length_aux*0.5 + self.ring_radius-self.ring_radius_aux-ring_wg_width - self.aux_coupling_gap, 0))
            insts += i3.SRef(name='bus_in_through', reference=self.bus_waveguide,
                             position=(self.coupler_positions[0], -self.ring_radius - ring_wg_width * 0.5 - self.coupler_spacing - bus_wg_width * 0.5))
            insts += i3.SRef(name='bus_add_drop', reference=self.bus_waveguide,
                             position=(self.coupler_positions[1], -self.ring_radius - ring_wg_width * 0.5 - self.coupler_spacing - bus_wg_width * 0.5)
                             )

            # RF electrode

            insts += i3.SRef(name='electrode', reference=self.rf_electrode,
                             position=(-150, self.ring_radius + self.electrode_gap * 0.5 + self.hot_width * 0.5))
